...
- Copilot Code Review second pass returned five more inline comments. All addressed in a follow-up commit on the same branch:
  ...
  - `tests/Feature/AuthTest.php` — new feature suite with seven tests covering: guest sees the login form, authenticated user is redirected away from `/login`, login succeeds and regenerates the CSRF token, login follows `url.intended`, login fails with invalid credentials, login validates email format and password presence, logout clears the session and redirects to `/login`.
