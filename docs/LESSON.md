...
- Demo seeders that create users with a well-known email/password should gate themselves to `local`/`testing` environments via `app()->environment()` and skip silently elsewhere. A demo seeder is by design idempotent and convenient, but those same properties make it a security footgun if it ever runs against staging/production: the user appears, the credentials are public knowledge, and nothing flags the situation. Keeping the gate inside the seeder method (rather than only at the artisan command level) means the safety holds even if a future call path invokes the seeder directly.
